...
def merge_server_params_with_local_bn(
    server_state_dict: dict, local_model_state_dict: dict
) -> dict:
    """
...
class CustomFedSiloBN(CustomFedAvg):
    ...
    def summary(self) -> None:
        ...
        log(
            INFO,
            "\t│\t└── BN statistics (running_mean/var): Siloed (never leave client)",
        )
    ...
    def configure_train(
        ...
        if server_round == 1:
            ...
            log(
                INFO,
                f"[SiloBN] Round {server_round}: Server model has {aggregatable_count} "
                f"aggregatable params and {bn_stat_count} BN statistics",
            )
            log(
                INFO,
                "[SiloBN] Note: BN statistics in server model are from initialization only",
            )
    ...
    def aggregate_train(
        ...
        if bn_stats_received:
            log(
                INFO,
                f"[SiloBN] WARNING: Received {len(bn_stats_received)} BN statistics "
                f"from clients. These should be filtered on client side!",
            )
            log(
                INFO,
                f"[SiloBN] BN stats received (will be ignored): {bn_stats_received[:5]}...",
            )

        log(
            INFO,
            f"[SiloBN] Round {server_round}: Aggregating {len(aggregatable_keys)} "
    ...
    #         log(
    #             INFO,
    #             f"[SiloBN] Round {actual_round}: Skipping evaluation (eval_interval={self.eval_interval})",
    #         )
    ...
    #         log(
    #             INFO,
    #             f"[SiloBN] Round {actual_round}: No evaluation replies (evaluation skipped)",
    #         )
    ...
    #         log(
    #             INFO,
    #             f"[SiloBN] Round {actual_round}: No valid evaluation metrics received",
    #         )
    #         return None, MetricRecord({})

    #     log(
    #         INFO,
    #         f"[SiloBN] Round {server_round}: Aggregating evaluation results from {len(client_metrics)} clients",
    #     )
